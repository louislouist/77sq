...
import {
        getEmitterCategoryInfo,
        findDesignationByICAO,
} from 'icao-designation';
import { getCurrentUTCwithUSDateTime } from '../etc/Dates';
...
export function buildAircraftInfoText(aircraft: Aircraft): string {
        ...
        if (aircraft.alt_geom !== undefined)
                info.push(`Altitude (Geom): ${aircraft.alt_geom} ft`);
        if (aircraft.gs !== undefined)
                info.push(`Ground Speed: ${aircraft.gs} knots`);
        if (aircraft.track !== undefined)
                info.push(`Track: ${aircraft.track}°`);
        if (aircraft.baro_rate !== undefined)
                info.push(`Climb/Descent Rate: ${aircraft.baro_rate} ft/min`);
        ...
        if (aircraft.lat !== undefined && aircraft.lon !== undefined) {
                info.push(
                        `Position: ${aircraft.lat.toFixed(4)}, ${aircraft.lon.toFixed(4)}`
                );
                ...
                info.push(
                        `Rough Position: ${aircraft.rr_lat.toFixed(4)}, ${aircraft.rr_lon.toFixed(4)}`
                );
                const airportData = getAirportInfo(
                        aircraft.rr_lat,
                        aircraft.rr_lon
                );
                airportData.forEach((line) => {
...
export function buildAircraftInfoTextRMD(aircraft: Aircraft): string {
        ...
        if (aircraft.flight && aircraft.flight.trim() != '') {
                ...
                linkInfo.push(
                        `[${aircraft.flight.trim()} on FightAware](${faUrl})`
                );
        }
        ...
        if (aircraft.r) {
                ...
                linkInfo.push(
                        `[${aircraft.r.trim()} on flightradar24](${frUrl})`
                );
        }
        ...
        if (
                ...
                (aircraft.rr_lat && aircraft.rr_lon)
        ) {
                const lat = aircraft.lat || aircraft.rr_lat;
                const lon = aircraft.lon || aircraft.rr_lon;
                linkInfo.push(
                        `[OSM Location](https://www.openstreetmap.org/#map=13/${lat}/${lon})`
                );
        }
        ...
        if (
                ...
                const lon = aircraft.lon || aircraft.rr_lon;
                linkInfo.push(
                        `[Google Maps](https://www.google.com/maps/search/airports/@${lat},${lon},11z)`
                );
        }
        ...
        const acInfo: string[] = [];
        if (aircraft.flight)
                acInfo.push(`***Flight***: ${aircraft.flight.trim()}`);
        ...
        if (aircraft.t) {
                ...
                info.push(
                        `***Aircraft***: ${aircraft.t}: ${winkInfo} [${aircraft.t} on doc8643.com](https://www.doc8643.com/aircraft/${aircraft.t})`
                );
        }

        if (aircraft.category) {
                const categoryDescription = getEmitterCategoryInfo(
                        aircraft.category.trim()
                );
                info.push(
                        `***Category***: ${aircraft.category}: (${categoryDescription})`
                );
        }
        ...
        if (aircraft.emergency)
                sqInfo.push(`***Emergency***: ${aircraft.emergency}`);
        if (aircraft.type)
                sqInfo.push(`***Message Type***: *${aircraft.type}*`);
        info.push(sqInfo.join(' '));

        info.push('## **Current Status:**');
        info.push(`***Date***: ${getCurrentUTCwithUSDateTime()}`);
        if (aircraft.alt_baro !== undefined) {
                if (aircraft.alt_baro === 'ground') {
                        info.push('***Altitude (Baro)***: Aircraft Landed');
                } else {
                        info.push(
                                `***Altitude (Baro)***: ${aircraft.alt_baro} ft`
                        );
                }
        }
        if (aircraft.alt_geom !== undefined)
        ...
        if (aircraft.baro_rate !== undefined)
                info.push(
                        `***Climb/Descent Rate***: ${aircraft.baro_rate} ft/min`
                );
        if (aircraft.nav_modes && aircraft.nav_modes.length > 0) {
                info.push(
                        `***Autopilot Modes***: ${aircraft.nav_modes.join(', ')}`
                );
        }
        ...
        if (closest.length > 0) {
                closest.forEach((airport) => {
                        ...
                        if (airport.wikipedia) {
                                airportInfo.push(
                                        ` [${airport.iata || airport.icao || airport.name} Wikipedia](${airport.wikipedia})\n`
                                );
                        }
                        if (icaoName && icaoName.trim() !== '') {
                                ...
                                if (liveATCExistsByICAO(icaoName.trim())) {
                                        airportInfo.push(
                                                ` [LiveATC @${icaoName}](http://www.liveatc.net/search/?icao=${icaoName})\n`
                                        );
                                }
                        } else {
                                airportInfo.push('\n');
